...
const getLanguageById = (lang: "c++" | "java" | "javascript" | "python") => {
    const language = {
        "c++": 54,
        "java": 62,
        "javascript": 63,
...
const submitToken = async (token: any[]) => {
    ...
    async function fetchData() {
        try {
            ...
            throw new Error("Error in fetching data from Judge0 API: " + error.message);
        }
    }
    while(true){
        const result = await fetchData();
        const isResult = result.submissions.every((res: any) => res.status_id > 2);
        if(isResult) {
